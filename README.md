...
- `PYTHONPATH` is set by `run12_app_start.bat` / `run12_app_start.sh` so the `src/` layout works from the repository root.
- The current run execution is a PoC background flow, not a full solver pipeline.
- Case updates require explicit approval.

## Operational Playbook

1. Start the server with `run12_app_start.bat` (Windows) or `run12_app_start.sh` (Linux/macOS) from the repository root so the `src/` package path is active.
